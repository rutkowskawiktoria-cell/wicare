...
- Built with the skills the owner named: **emilkowalski/skills** (design-eng rules + prototype PICKER, copied verbatim), **pbakaus/impeccable** (anti-pattern detector; CLI `npx impeccable detect --json <dir>` — prototypes = 0 findings; live baseline had 1: bounce easing on the hero chevron), **leonxlnx/taste-skill**. Figma MCP needs the owner to authorise it in connector settings; Playwright MCP wasn't connected (no headless browser in sandbox) → tested in the in-app Browser instead.
- Preview: **https://redesign-wicare.rutkowska-wiktoria.workers.dev/prototypes/homepage/** (`?v=1|2|3`, keys 1–3 / ←→, R = replay). Page is `noindex`, not linked, not in sitemap.
- Code: `src/app/prototypes/homepage/` — `page.tsx` (fonts: Bodoni Moda, Albert Sans, Familjen Grotesk, Jost via next/font), `Harness.tsx` (picker), `shared.tsx` (nav, mobile call bar, contact form = same Web3Forms/Gmail logic, footer), `copy.ts` (DA/EN, sentence case, no new claims), `variants/{Porcelain,Coast,Concierge}.tsx` + one CSS file each. Production components/translations untouched.
- Directions: **1 Porcelæn** (editorial, Bodoni + fluted-porcelain ornament, light), **2 Kystlys** (cinematic, navy-drenched full-bleed photos, scroll-driven clip reveals), **3 Concierge** (hero asks "Hvad kan vi tage os af?" → key-tag selector swaps service/photo/CTAs; WhatsApp pre-filled per service; directory-style areas list).
- All keep `id="booking"` on the hero (printed-card QR), plus `#services #trust #testimonials #contact #book`.
- Gotchas: Next does **not** keep CSS import order between files → shared resets use `:where()` (zero specificity). Don't use the `background:` shorthand on `.pf-input` (it wipes the select chevron's no-repeat). The in-app Browser pane stops repainting when Claude's window is covered — toggling the viewport size by 1px forces a fresh frame before a screenshot.
- Next step once the owner picks: promote the chosen direction into the real components (`page.tsx`, sections, translations), then extend to service/area pages, via `dev` → CI → `main`.
