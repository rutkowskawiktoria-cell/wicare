...
- **Hero headline (live, Sept 2026):** title renders on three lines — `VIP` / `Home & Lifestyle` (DA `Hjem & Livsstil`) / `Services` — via `\n` in `hero.title` + `whitespace-pre-line` on the `<h1>`; the motto (`hero.motto`: "Discretion. Precision. Trust." / "Diskretion. Præcision. Tillid.") is its own key, rendered bold before `hero.subtitle`. The "delivered with care and precision" line (`titleHighlight`) was removed.
- **Contact + booking merged (Sept 2026):** the homepage no longer has a separate contact section. `Booking.tsx` (`id="book"`) is a two-column navy section — left: "Klar til at komme i gang?", phone button, 3 compact stats, valuables note; right: `ContactForm.tsx`, now a white card (`id="contact"`, `scroll-mt-24`) rendered inside Booking. Form intro changed to "Foretrækker du at skrive? …" to avoid repeating the 2-hour promise. The van photo was dropped from this section.
- **Trust + testimonials merged (Sept 2026):** `Trust.tsx` (`id="trust"`, bg-light) is two columns — left: "Hvorfor vælge os / WiCare-forskellen" with the 4 reasons as a check-list; right: `Testimonials.tsx`, now a column component (`id="testimonials"`, `scroll-mt-24`) with the 3 review cards stacked. `page.tsx` order: Hero → Services → Trust → Booking.
